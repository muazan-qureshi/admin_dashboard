...
class egan
{
    ...
    function userlogin()
    {
        if (isset($_POST['login_btn'])) {
            ...
            $sql = "SELECT `id`, `fullname`, `email`, `password` FROM `users` WHERE email = '$email' AND password ='$password'";
    ...
    {
        $a = $_SESSION['log'];
        $query = mysqli_query($this->connection(), "SELECT * FROM `users` where email = '$a' ");
        return $query;
